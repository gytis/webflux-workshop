...
This repository hosts a complete workshop on Spring Boot + Spring WebFlux + Vert.x.
...
Edit your `pom.xml` file to replace `org.springframework.boot:spring-boot-starter-webflux` dependency with `me.snowdrop:vertx-spring-boot-http:0.0.1-SNAPSHOT`

[source,xml]
.stock-quotes/pom.xml
----
include::../stock-quotes/pom.xml[tags=vertx]
----
...
Launching it from your IDE or with `mvn spring-boot:run` should start a Vert.x server on port 8081.
...
----
include::../trading-service/pom.xml[tags=vertx]
----
